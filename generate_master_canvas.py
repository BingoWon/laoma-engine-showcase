...
<head>
  ...
  <title>老马喵务局 · 端侧智能体全景工程拆解与技术资产总成</title>
  <!-- Tailwind CSS CDN -->
  ...
  <style>
    ...
    /* Unified square squircle containers */
    .avatar-squircle {
      width: 52px;
      height: 52px;
      aspect-ratio: 1 / 1;
...
<body class="bg-slate-100 text-slate-800 antialiased overflow-hidden font-sans h-screen w-screen flex flex-col select-none">
  ...
  <header class="fixed top-4 left-6 right-6 z-50 pointer-events-none flex items-center justify-between">
    ...
    <div class="pointer-events-auto flex items-center gap-3 bg-white/95 backdrop-blur-xl px-4 py-2 rounded-2xl border border-slate-200/90 shadow-sm">
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          <span class="text-[10px] px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 font-bold border border-blue-200/60 font-mono">CORE-V2.4</span>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 font-mono font-bold">100% 全量物料归档</span>
        </div>
        <p class="text-[10.5px] text-slate-400 font-medium mt-0.5 font-mono">
          MASTER PRECISION BLUEPRINT · 产研工模 · 视觉选型候选库 · 控制总线
        </p>
      </div>
    </div>
    ...
    <div class="pointer-events-auto flex items-center gap-2 bg-white/95 backdrop-blur-xl px-3 py-2 rounded-2xl border border-slate-200/90 shadow-sm">
      <span class="text-xs font-mono font-black text-slate-700">SCALE: <span id="hud-zoom" class="text-blue-600">75%</span></span>
  ...
  <main id="canvas-viewport" class="w-full h-full relative overflow-hidden bg-blueprint-grid cursor-grab active:cursor-grabbing">
    ...
    <!-- Giant Stage Layer: 6800px × 4800px (All Zones Arranged Organically) -->
    <div id="canvas-stage" class="absolute pointer-events-auto" style="width: 6800px; height: 4800px;">
      ...
      <!-- ===================================================================== -->
      <!-- ZONE NORTH (顶部核心架构 · Y: 750) · 系统技术栈架构、控制系统与 FSM 状态机拓扑 -->
      <!-- ===================================================================== -->
      <div id="zone-north" class="absolute z-20 bg-white/95 backdrop-blur-md p-8 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 2300px; top: 750px; width: 2200px;">
      ...
      <!-- ===================================================================== -->
      <!-- NEW ZONE NORTHWEST (西北总成 · X: 840, Y: 750) · 实体猫咪真实物理建模基准库 -->
      <!-- ===================================================================== -->
      <div id="zone-reference" class="absolute z-20 bg-white/95 backdrop-blur-md p-7 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 840px; top: 750px; width: 1360px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-3 mb-5">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <h3 class="font-black text-slate-900 text-sm">实体猫咪真实物理建模基准库 (12组实录高保真特征样张)</h3>
              <span class="text-[10px] font-mono text-amber-700 font-bold">[REF-LIB-REAL · PHYSICAL PROTOTYPING & SKELETAL REFERENCE]</span>
            </div>
          </div>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 font-mono font-bold">12 REFERENCE SAMPLES</span>
        </div>

        <!-- 12 Real Photos Unified Square Grid (2 Rows x 6 Cols) -->
        <div class="grid grid-cols-6 gap-3">
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_1.jpg', '物理样张 01 · 蓝眼折射与正坐骨骼')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_1.jpg" class="w-full h-full object-cover" alt="实测1"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_01</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_2.jpg', '物理样张 02 · 伏卧工况与耳廓解剖')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_2.jpg" class="w-full h-full object-cover" alt="实测2"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_02</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_3.jpg', '物理样张 03 · 头部特写与眼周毛流')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_3.jpg" class="w-full h-full object-cover" alt="实测3"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_03</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_4.jpg', '物理样张 04 · 办公桌自然光照环境')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_4.jpg" class="w-full h-full object-cover" alt="实测4"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_04</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_5.jpg', '物理样张 05 · 站立侧身比例基准')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_5.jpg" class="w-full h-full object-cover" alt="实测5"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_05</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_6.jpg', '物理样张 06 · 舒展放松腹部特征')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_6.jpg" class="w-full h-full object-cover" alt="实测6"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_06</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_real_7.jpg', '物理样张 07 · 侧躺伸展运动学采样')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_real_7.jpg" class="w-full h-full object-cover" alt="实测7"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_REAL_07</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_photo_1.jpg', '物理样张 08 · 箱体内空间透视参考')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_photo_1.jpg" class="w-full h-full object-cover" alt="实测8"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_PHOTO_01</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_photo_2.jpg', '物理样张 09 · 阳光漫反射光影采样')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_photo_2.jpg" class="w-full h-full object-cover" alt="实测9"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_PHOTO_02</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_photo_3.jpg', '物理样张 10 · 近景鼻吻部微观纹理')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_photo_3.jpg" class="w-full h-full object-cover" alt="实测10"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_PHOTO_03</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_photo_4.jpg', '物理样张 11 · 爪垫与握爪触控参照')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_photo_4.jpg" class="w-full h-full object-cover" alt="实测11"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_PHOTO_04</div>
          </div>
          <div class="bg-slate-50 p-2 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-400 transition-all" onclick="inspectImage('./assets/real_photos/laoma_photo_5.jpg', '物理样张 12 · 睡眠工况呼吸微起伏基准')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/real_photos/laoma_photo_5.jpg" class="w-full h-full object-cover" alt="实测12"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">REF_PHOTO_05</div>
          </div>
        </div>
      </div>

      <!-- ===================================================================== -->
      <!-- NEW ZONE NORTHEAST (东北总成 · X: 4600, Y: 750) · 脏污多工况算法训练集与候选实验池 -->
      <!-- ===================================================================== -->
      <div id="zone-candidates" class="absolute z-20 bg-white/95 backdrop-blur-md p-7 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 4600px; top: 750px; width: 1460px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-3 mb-5">
          <div class="flex items-center gap-2.5">
            <span class="w-8 h-8 rounded-xl bg-cyan-50 border border-cyan-200 flex items-center justify-center text-base">🔬</span>
            <div>
              <h3 class="font-black text-slate-900 text-sm">脏污工况与特征提取候选池 (多模态视觉选型实验)</h3>
              <span class="text-[10px] font-mono text-cyan-700 font-bold">[EXP-CANDIDATES · DIRTY STATE CV TRAINING & VARIANT POOL]</span>
            </div>
          </div>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-cyan-50 text-cyan-700 font-mono font-bold">10 CANDIDATE ITERATIONS</span>
        </div>

        <!-- 10 Candidates Grid (2 Rows x 5 Cols) -->
        <div class="grid grid-cols-5 gap-3">
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/dirty_cat_candidate_1.png', '脏污候选 01 · 机箱灰斑特征聚类')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/dirty_cat_candidate_1.png" class="w-full h-full object-cover" alt="候选1"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">DIRTY_CANDIDATE_01</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/dirty_cat_candidate_2.png', '脏污候选 02 · 脸颊泥渍分布模型')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/dirty_cat_candidate_2.png" class="w-full h-full object-cover" alt="候选2"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">DIRTY_CANDIDATE_02</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/dirty_cat_candidate_3.png', '脏污候选 03 · 全身尘屑高反差样张')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/dirty_cat_candidate_3.png" class="w-full h-full object-cover" alt="候选3"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">DIRTY_CANDIDATE_03</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/office_dirty_cat_candidate_1.png', '办公区候选 01 · 桌面漫散射打光')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/office_dirty_cat_candidate_1.png" class="w-full h-full object-cover" alt="候选4"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">OFFICE_DIRTY_01</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/office_dirty_cat_candidate_2.png', '办公区候选 02 · 暖白温平衡校正')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/office_dirty_cat_candidate_2.png" class="w-full h-full object-cover" alt="候选5"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">OFFICE_DIRTY_02</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/office_dirty_cat_candidate_3.png', '办公区候选 03 · 阴影遮蔽区域测光')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/office_dirty_cat_candidate_3.png" class="w-full h-full object-cover" alt="候选6"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">OFFICE_DIRTY_03</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/dirty_cat_sample_1.png', '灰阶采样 01 · 鼻吻部灰度衰减')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/dirty_cat_sample_1.png" class="w-full h-full object-cover" alt="候选7"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">DIRTY_SAMPLE_01</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/dirty_cat_sample_2.png', '灰阶采样 02 · 耳廓边缘污染模拟')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/dirty_cat_sample_2.png" class="w-full h-full object-cover" alt="候选8"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">DIRTY_SAMPLE_02</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/dirty_cat_sample_3.png', '灰阶采样 03 · 最终选型定型基准')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/dirty_cat_sample_3.png" class="w-full h-full object-cover" alt="候选9"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">DIRTY_SAMPLE_03</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-cyan-400 transition-all" onclick="inspectImage('./assets/candidates/app_icon_preview_orig.png', '图标原版对比 · 色彩空间未校准样张')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/candidates/app_icon_preview_orig.png" class="w-full h-full object-cover" alt="候选10"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">APP_ICON_RAW_UNP</div>
          </div>
        </div>
      </div>

      <!-- ===================================================================== -->
      <!-- ZONE CENTER (中央整机装配体与发动机有机拆解核心 · Y: 1950) -->
      <!-- ===================================================================== -->

      <!-- 1. 中央整机装配体 (The Central Core: Honor Native Target) -->
      <div id="core-device-frame" class="absolute z-30" style="left: 3205px; top: 1950px; width: 390px; height: 844px;">
      ...
      <div id="module-gauges" class="exploded-module absolute z-20 bg-white/95 backdrop-blur-md p-5 rounded-3xl border border-cyan-200/80 shadow-xl glow-cyan cursor-pointer" style="left: 2580px; top: 1520px; width: 520px;" onclick="inspectComponent('gauges')">
      ...
      <div id="module-media" class="exploded-module absolute z-20 bg-white/95 backdrop-blur-md p-5 rounded-3xl border border-emerald-200/80 shadow-xl glow-emerald cursor-pointer" style="left: 3720px; top: 1520px; width: 540px;" onclick="inspectComponent('media')">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
          <div class="flex items-center gap-2.5">
      ...
      <div id="module-timeline" class="exploded-module absolute z-20 bg-white/95 backdrop-blur-md p-5 rounded-3xl border border-indigo-200/80 shadow-xl glow-indigo cursor-pointer" style="left: 2440px; top: 1980px; width: 660px;" onclick="inspectComponent('timeline')">
      ...
      <div id="module-bento" class="exploded-module absolute z-20 bg-white/95 backdrop-blur-md p-5 rounded-3xl border border-amber-200/80 shadow-xl glow-amber cursor-pointer" style="left: 3720px; top: 1980px; width: 560px;" onclick="inspectComponent('bento')">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
          <div class="flex items-center gap-2.5">
            <span class="w-8 h-8 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center text-base">📐</span>
            <div>
            ...
            <div class="relative w-full h-24 my-2 flex items-end justify-center">
              <div class="absolute inset-0 flex items-center justify-around z-20">
                <div class="flex flex-col items-center">
                  <div class="w-7 h-7 rounded-full overflow-hidden ring-2 ring-slate-200"><img src="./assets/app_assets/avatars/avatar_xiaobai.png" class="w-full h-full object-cover" alt="晓白"></div>
                  <span class="text-[9px] font-bold text-slate-600 mt-0.5">RANK 02</span>
                </div>
                <div class="flex flex-col items-center -translate-y-2">
                  <div class="w-8 h-8 rounded-full overflow-hidden ring-2 ring-amber-400"><img src="./assets/app_assets/avatars/avatar_lex.png" class="w-full h-full object-cover" alt="Lex"></div>
                  <span class="text-[10px] font-black text-slate-800 mt-0.5 font-mono">TOP 01 👑</span>
                </div>
                <div class="flex flex-col items-center">
                  <div class="w-7 h-7 rounded-full overflow-hidden ring-2 ring-amber-700/40"><img src="./assets/app_assets/avatars/avatar_linyi.png" class="w-full h-full object-cover" alt="林亦"></div>
      ...
      <!-- ===================================================================== -->
      <!-- ZONE EAST (东侧总成 · X: 4380, Y: 1520) · 8 组真机运行工况验证矩阵 -->
      <!-- ===================================================================== -->
      <div id="zone-screens" class="absolute z-20 bg-white/95 backdrop-blur-md p-8 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 4380px; top: 1520px; width: 1680px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-4 mb-6">
          <div class="flex items-center gap-3">
        ...
        <div class="grid grid-cols-4 gap-6">
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174540_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况1" onclick="inspectImage(this.src, 'RUN-01: 初始全景渲染装配态 (17:45:40)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174600_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况2" onclick="inspectImage(this.src, 'RUN-02: 物理手势阻尼位移展开 (17:46:00)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174613_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况3" onclick="inspectImage(this.src, 'RUN-03: 触控多点采样与动效响应 (17:46:13)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174628_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况4" onclick="inspectImage(this.src, 'RUN-04: FSM 指标动态重算工况 (17:46:28)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174648_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况5" onclick="inspectImage(this.src, 'RUN-05: 传感器阈值中断触发 (17:46:48)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174700_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况6" onclick="inspectImage(this.src, 'RUN-06: 3D 领奖台二级页面渲染 (17:47:00)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174713_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况7" onclick="inspectImage(this.src, 'RUN-07: 时序结构化存储容器展开 (17:47:13)')">
          ...
          <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200">
            <img src="./assets/screenshots/Screenshot_20260919_174724_com_lyi_laomayoyo_MainActivity.JPG" class="w-full h-auto rounded-xl shadow-xs mb-2 cursor-pointer hover:scale-105 transition-transform" alt="工况8" onclick="inspectImage(this.src, 'RUN-08: 自适应高度卡片排版计算 (17:47:24)')">
      ...
      <!-- ===================================================================== -->
      <!-- ZONE WEST (西侧总成 · X: 840, Y: 1520) · 全量工程零部件物料清单 (BOM) -->
      <!-- ===================================================================== -->
      <div id="zone-bom" class="absolute z-20 bg-white/95 backdrop-blur-md p-8 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 840px; top: 1520px; width: 1500px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-4 mb-6">
          <div class="flex items-center gap-3">
      ...
      <!-- ===================================================================== -->
      <!-- NEW ZONE SOUTHWEST (西南总成 · X: 840, Y: 2950) · 时序视频动作捕捉与关键帧定格总线 -->
      <!-- ===================================================================== -->
      <div id="zone-posters" class="absolute z-20 bg-white/95 backdrop-blur-md p-7 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 840px; top: 2950px; width: 1360px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-3 mb-5">
          <div class="flex items-center gap-2.5">
            <span class="w-8 h-8 rounded-xl bg-indigo-50 border border-indigo-200 flex items-center justify-center text-base">🎞️</span>
            <div>
              <h3 class="font-black text-slate-900 text-sm">时序动作捕捉与视频关键帧采样总线 (6组高帧率行为流)</h3>
              <span class="text-[10px] font-mono text-indigo-700 font-bold">[VID-POSTERS · TEMPORAL MOTION CAPTURE & KEYFRAME SAMPLING]</span>
            </div>
          </div>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-mono font-bold">6 MOTION STREAMS</span>
        </div>

        <!-- 6 Video Posters Grid (1 Row x 6 Cols) -->
        <div class="grid grid-cols-6 gap-3">
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-400 transition-all" onclick="inspectImage('./assets/video_posters/vid_cat_controlled_poster.jpg', '动作帧 01 · 抱持受控动作流定格')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/video_posters/vid_cat_controlled_poster.jpg" class="w-full h-full object-cover" alt="视频帧1"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">MOTION_CONTROLLED</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-400 transition-all" onclick="inspectImage('./assets/video_posters/vid_cat_feast_poster.jpg', '动作帧 02 · 进食咀嚼肌肉运动捕捉')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/video_posters/vid_cat_feast_poster.jpg" class="w-full h-full object-cover" alt="视频帧2"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">MOTION_FEAST</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-400 transition-all" onclick="inspectImage('./assets/video_posters/vid_cat_charming_belly_poster.jpg', '动作帧 03 · 翻肚撒娇高动态范围')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/video_posters/vid_cat_charming_belly_poster.jpg" class="w-full h-full object-cover" alt="视频帧3"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">MOTION_BELLY</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-400 transition-all" onclick="inspectImage('./assets/video_posters/vid_cat_taste_after_poster.jpg', '动作帧 04 · 舔舐梳理舌面微结构')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/video_posters/vid_cat_taste_after_poster.jpg" class="w-full h-full object-cover" alt="视频帧4"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">MOTION_TASTE</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-400 transition-all" onclick="inspectImage('./assets/video_posters/vid_cat_attack_slacker_poster.jpg', '动作帧 05 · 扑咬微动作惯性测定')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/video_posters/vid_cat_attack_slacker_poster.jpg" class="w-full h-full object-cover" alt="视频帧5"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">MOTION_ATTACK</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-400 transition-all" onclick="inspectImage('./assets/video_posters/vid_cat_fire_extinguisher_poster.jpg', '动作帧 06 · 突发好奇应激姿态采样')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/video_posters/vid_cat_fire_extinguisher_poster.jpg" class="w-full h-full object-cover" alt="视频帧6"></div>
      ...
      <!-- ===================================================================== -->
      <!-- ZONE SOUTH (南侧总成 · Y: 2950) · 产研 14 人工程职能与代码交付架构 (Strictly Aligned Avatars) -->
      <!-- ===================================================================== -->
      <div id="zone-team" class="absolute z-20 bg-white/95 backdrop-blur-md p-8 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 2300px; top: 2950px; width: 2200px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-4 mb-6">
          <div class="flex items-center gap-3">
            <span class="w-10 h-10 rounded-2xl bg-blue-50 border border-blue-200 flex items-center justify-center text-xl">👥</span>
            <div>
        ...
        <!-- 14 Engineers Grid (Strictly Aligned Squircles) -->
      ...
      <!-- ===================================================================== -->
      <!-- NEW ZONE SOUTHEAST (东南总成 · X: 4600, Y: 2950) · 状态微纹理与场景材质着色器候选池 -->
      <!-- ===================================================================== -->
      <div id="zone-shaders" class="absolute z-20 bg-white/95 backdrop-blur-md p-7 rounded-3xl border border-slate-300/80 shadow-xl" style="left: 4600px; top: 2950px; width: 1460px;">
        <div class="flex items-center justify-between border-b border-slate-200 pb-3 mb-5">
          <div class="flex items-center gap-2.5">
            <span class="w-8 h-8 rounded-xl bg-rose-50 border border-rose-200 flex items-center justify-center text-base">🎨</span>
            <div>
              <h3 class="font-black text-slate-900 text-sm">状态微纹理与场景材质着色器候选池 (8组材质着色试验)</h3>
        ...
        <div class="grid grid-cols-4 gap-3">
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/generated/status_bath.png', '材质实验 01 · 气泡水流折射着色器')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/generated/status_bath.png" class="w-full h-full object-cover" alt="材质1"></div>
          ...
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/generated/status_food.png', '材质实验 02 · 颗粒质感漫散射着色器')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/generated/status_food.png" class="w-full h-full object-cover" alt="材质2"></div>
          ...
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/generated/status_water.png', '材质实验 03 · 水波次表面散射')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/generated/status_water.png" class="w-full h-full object-cover" alt="材质3"></div>
          ...
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/generated/status_litter.png', '材质实验 04 · 矿物颗粒凹凸贴图')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/generated/status_litter.png" class="w-full h-full object-cover" alt="材质4"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">SHADER_LITTER_BUMP</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/generated/btn_rank_art.png', '材质实验 05 · 金属高光浮雕微纹理')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/generated/btn_rank_art.png" class="w-full h-full object-cover" alt="材质5"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">TEXTURE_RANK_METAL</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/generated/btn_archive_art.png', '材质实验 06 · 手作纸质漫反射微纹理')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/generated/btn_archive_art.png" class="w-full h-full object-cover" alt="材质6"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">TEXTURE_ARCHIVE_PAPER</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/scenes/leaderboard_magic_glass.png', '材质实验 07 · 毛玻璃光学模糊视效')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/scenes/leaderboard_magic_glass.png" class="w-full h-full object-cover" alt="材质7"></div>
            <div class="text-[10px] font-bold text-slate-700 font-mono truncate">SCENE_FROSTED_GLASS</div>
          </div>
          <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-200 cursor-pointer hover:border-rose-400 transition-all" onclick="inspectImage('./assets/app_assets/scenes/leaderboard_cozy_journal.png', '材质实验 08 · 暖色调环境光烘焙样张')">
            <div class="thumb-squircle mb-1.5"><img src="./assets/app_assets/scenes/leaderboard_cozy_journal.png" class="w-full h-full object-cover" alt="材质8"></div>
  ...
  <script>
    ...
    let panX = -2050; // default centered at central phone
    let panY = -1550;
    let zoomScale = 0.75;
    ...
    // -------------------------------------------------------------------------
    const ZONE_COORDS = {
      center: { x: 3400, y: 2370, zoom: 0.8 },
      screens: { x: 5220, y: 1950, zoom: 0.7 },
      candidates: { x: 5330, y: 1100, zoom: 0.75 },
      reference: { x: 1520, y: 1100, zoom: 0.75 },
      team: { x: 3400, y: 3250, zoom: 0.7 },
      bom: { x: 1590, y: 1950, zoom: 0.7 },
      north: { x: 3400, y: 1100, zoom: 0.75 }
    };
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-1.5">
                  <div class="w-4 h-4 rounded-full overflow-hidden shrink-0"><img src="${it.avatar}" class="w-full h-full object-cover" alt="${it.person}"></div>
      ...
      TEAM_MEMBERS.forEach(m => {
        html += `
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 hover:border-blue-500 hover:bg-white transition-all flex flex-col items-center text-center cursor-pointer" onclick="inspectMember('${m.alias}')">
            <!-- Strictly Aligned Square Squircle Avatar (Fixed 52x52) -->
            <div class="avatar-squircle shadow-xs ring-2 ring-slate-200 mb-2.5 bg-slate-200">
              <img src="${m.avatar}" class="w-full h-full object-cover" alt="${m.alias}">
            </div>
            ...
            <span class="text-[9px] text-slate-400 mt-1.5 leading-relaxed font-mono">${m.desc}</span>
          </div>
        `;
      });
      container.innerHTML = html;
    ...
    function inspectImage(src, title) {
      ...
      document.getElementById('insp-type').textContent = 'High-Precision Asset Sample';
      document.getElementById('insp-author').textContent = '工程资产归档库';
      document.getElementById('insp-desc').textContent = '系统全量素材与选型实验样张，作为视觉建模、多模态采样及状态机触发判定的真实工程依据。';
      drawer.classList.remove('translate-x-[460px]');
    }
...
print("Master Canvas index.html updated successfully!")
